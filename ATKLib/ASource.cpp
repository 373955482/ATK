...
void ASource::StartCmd()
{
   ...
   if (fmt == HAUDIO){
      timeNow = GetTimeNow();
      SendMarkerPkt("START");printf("open audio device here!");
...
{  printf("stop cmd 1\n");
   if (fmt == HAUDIO){
      StopAudioInput(ain);printf("stop cmd 2\n");
      SendMarkerPkt("PAUSE");
   }
   stopping = TRUE;printf("stop cmd 3\n");
   if (showVM) DrawButton();printf("stop cmd 4\n");
}
   ...
   try{
      ...
      while (!asp->IsTerminated()){
         ...
         //if (asp->stopped || HEventsPending(0)){
            ...
            //case HTBUFFER:
             //  if (e.c == MSGEVENTID) {
			//	   asp->ChkMessage(); printf("Checked Msg here!637\n");
