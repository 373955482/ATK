...
int main(int argc, char *argv[])
{
   ...
   try {
	  ...
	  printf("asrc->Start();1\n");
      printf("Starting Audio Source Test here\n");
      asrc->Start();
	  printf("asrc->Start();02");
	  //asrc->StartRecording();
      if (startImm) 
		  asrc->SendMessage("start()");
	  printf("asrc->SendMessage start()3");
	  //asrc->SendMessage("start()");
	  //asrc->StartRecording();
	  int Tcount = 0;
            ...
            //if (tnow > beepTime) {
               if (maxcycles>0) {
                  ...
                  if (numcycles>maxcycles) {
					  asrc->SendMessage("stop()"); SendOutput(outbuf, outsize); printf("timeout here\n");
