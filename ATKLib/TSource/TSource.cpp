...
int main(int argc, char *argv[])
{
   ...
   try {
      ...
      asrc->Start();
	  printf("asrc->Start();");
	  //if (startImm) asrc->SendMessage("start()"); printf("asrc->SendMessage start");
	  asrc->SendMessage("start()");
	  //asrc->StartRecording();
	  int Tcount = 0;
	  string s = GetStringPacket();
	  while (numcycles<maxcycles) //(!asrc->IsTerminated())
	  {
      ...
      //            if (numcycles>maxcycles) 
				  //{
					 // asrc->SendMessage("stop()");// SendOutput(outbuf, outsize); 
      //               asrc->SendMessage("terminate()");printf("timeout here\n");
      ...
      //            }
         ...
         //if (e.event == HTBUFFER) {
            ...
            //case INBUFID:
               //if (IsStringPacket()) {
				   //string s = GetStringPacket(); printf("get string %s\n",s);
      //            if (s=="ASource::START"){printf("start here\n");
      //            }else if (s=="ASource::STOP") {
					 //// SendOutput(outbuf, outsize); printf("stop here\n");
					 //outsize = 0; 
      //            }else if (s=="ASource::TERMINATE") {
      //               //printf("Source terminated\n");
      //            }else {
					 // //printf("Custom string marker: %s\n", s.c_str()); printf("terminated here\n");
      //            }
              // } else {
                  //GetWavePacket();printf("get wavdata here %d\n", Tcount);
               //}
              // break;
            //case ACKBUFID:
            //   if (! ackBuf->IsEmpty()){
